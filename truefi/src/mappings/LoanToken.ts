...
import { createBorrowerStableRate } from "../entities/rate";
import { getOrCreateToken, getTokenById } from "../entities/token";
...
export function handleWithdrawn(event: Withdrawn): void {
  ...
  if (!apyResult.reverted) {
    const newRate = createBorrowerStableRate(
      poolAddress,
      apyResult.value.toBigDecimal().div(BIGDECIMAL_HUNDRED),
      timestamp
    );
    updateMarketRates(event, market, [newRate.id]);
  }
