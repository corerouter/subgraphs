...
import { SECONDS_PER_HOUR, SECONDS_PER_DAY, GENESIS_ADDRESS, BIGINT_ZERO, BIGINT_ONE} from '../common/constants'
...
function handleTransferEvent(
  ...
  if (token != null) {
    ...
    let dailySnapshot = getOrCreateTokenDailySnapshot(token, event.block)
    dailySnapshot.dailyHolderCount = dailySnapshot.dailyHolderCount.plus(newAccountNum)
    dailySnapshot.dailyActiveUsers = dailySnapshot.dailyActiveUsers.plus(newAccountNum)
    ...
    let hourlySnapshot = getOrCreateTokenHourlySnapshot(token, event.block)
    hourlySnapshot.hourlyHolderCount = hourlySnapshot.hourlyHolderCount.plus(newAccountNum)
    hourlySnapshot.hourlyActiveUsers = hourlySnapshot.hourlyActiveUsers.plus(newAccountNum)
...
function getOrCreateTokenDailySnapshot(token: Token, block: ethereum.Block): TokenDailySnapshot {
  ...
  newSnapshot.dailyActiveUsers = BigInt.fromI32(0)
  newSnapshot.cumulativeUniqueUsers = token.holderCount
  newSnapshot.dailyEventCount = 0
  newSnapshot.dailyTransferCount = 0
  newSnapshot.dailyTransferAmount = BigInt.fromI32(0).toBigDecimal()
  newSnapshot.dailyMintCount = 0
  newSnapshot.dailyMintAmount = BigInt.fromI32(0).toBigDecimal()
  newSnapshot.dailyBurnCount = 0
  newSnapshot.dailyBurnAmount = BigInt.fromI32(0).toBigDecimal()
...
function getOrCreateTokenHourlySnapshot(token: Token, block: ethereum.Block): TokenHourlySnapshot {
  ...
  newSnapshot.hourlyActiveUsers = BigInt.fromI32(0)
  newSnapshot.cumulativeUniqueUsers = token.holderCount
  newSnapshot.hourlyEventCount = 0
  newSnapshot.hourlyTransferCount = 0
  newSnapshot.hourlyTransferAmount = BigInt.fromI32(0).toBigDecimal()
  newSnapshot.hourlyMintCount = 0
  newSnapshot.hourlyMintAmount = BigInt.fromI32(0).toBigDecimal()
  newSnapshot.hourlyBurnCount = 0
  newSnapshot.hourlyBurnAmount = BigInt.fromI32(0).toBigDecimal()
