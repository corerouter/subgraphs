...
export function createToken(value: JSONValue, userData: Value): void {
  ...
  if (address != null) {
    ...
    if (token == null) {
      let erc20Token = ERC20.bind(contractAddress)
      let initialSupply = erc20Token.try_totalSupply()
      ...
      token.name = tokenName.reverted ? name : tokenName.value
      token.symbol = tokenSymbol.reverted ? symbol : tokenSymbol.value
      token.decimals = tokenDecimals.reverted ? decimals : tokenDecimals.value
      ...
      token.totalSupply = initialSupply.reverted ? totalSupply : toDecimal(initialSupply.value, token.decimals)
      log.info('initialSupply.reverted: {}', [initialSupply.reverted.toString()])
