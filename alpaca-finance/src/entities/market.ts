...
import {
  ...
  updateProtocolTVL,
  addProtocolSideRevenueWithoutSnapshot,
  addSupplySideRevenueWithoutSnapshot,
} from "./protocol";
...
import { updateInterest, updateRewardTokens } from "../mappings/Vault";
import {
...
export function createMarket(
  ...
  market._reservePool = BIGINT_ZERO;
...
export function addMarketSupplySideRevenueWithoutSnapShot(
  event: ethereum.Event,
  market: Market,
  ...
  market.save();
  addSupplySideRevenueWithoutSnapshot(event, revenueAmountUSD);
}
...
export function addMarketProtocolSideRevenueWithoutSnapshot(
  event: ethereum.Event,
  market: Market,
  revenueAmountUSD: BigDecimal
): void {
  ...
  market.cumulativeTotalRevenueUSD =
    market.cumulativeTotalRevenueUSD.plus(revenueAmountUSD);
  market.save();
  addProtocolSideRevenueWithoutSnapshot(event, revenueAmountUSD);
}
...
function updateMarketTVL(event: ethereum.Event, market: Market): void {
  ...
  if (totalValueLockedUSD.ge(BIGDECIMAL_ZERO)) {
    ...
    const totalDepositBalanceUSD = new BigDecimal(market.inputTokenBalance)
      .div(exponentToBigDecimal(inputToken.decimals))
      .times(inputTokenPriceUSD);
    updateProtocolTVL(
      event,
      totalValueLockedUSD.minus(market.totalValueLockedUSD),
      totalDepositBalanceUSD.minus(market.totalDepositBalanceUSD)
    );
