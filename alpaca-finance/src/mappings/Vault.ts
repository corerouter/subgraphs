...
import {
  ...
  createMarket,
  getOrCreateMarket,
  addMarketProtocolSideRevenueWithoutSnapshot,
  addMarketSupplySideRevenueWithoutSnapShot,
...
export function updateInterest(event: ethereum.Event, market: Market): void {
  ...
  //log.warning("!tryReservePool.reverted", []);
  // let reservePoolChangeAmount = tryReservePool.value.minus(market._reservePool);
  // if (reservePoolChangeAmount.gt(BIGINT_ZERO)) {
  //   const protocolSideProfitUSD = amountInUSD(
  //     reservePoolChangeAmount,
  ...
  // }
  // market._reservePool = tryReservePool.value;
  ...
  if (
    ...
    //log.warning("!tryConfig.reverted", []);
    ...
    // log.warning(
    //   "!tryGetInterestRate.reverted {} with tryVaultDebtVal {} and market.inputTokenBalance {}",
    //   [
    //     tryGetInterestRate.reverted.toString(),
    //     tryVaultDebtVal.value.toString(),
